...
import { deleteNoteAction } from "@/src/actions/notes";
...
function DeleteNoteButton({noteId, deleteNoteLocally}: Props) {
  ...
  const handleDeleteNote = () => {
    startTransition(async () => {
      const {errorMessage, newestNoteId} = await deleteNoteAction(noteId);
      if (!errorMessage) {
  ...
  return (
    <AlertDialog>
      <AlertDialogTrigger asChild>
        <Button className="absolute right-2 top-1 -translate-y-0.5 
        size-7 p-0 opacity-0 group-hover/menu-item:opacity-100 [&_svg]:size-3"
        variant="ghost"
        >
          <Trash2 />
        </Button>
